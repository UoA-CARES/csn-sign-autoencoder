...
class CSNBottleneck(nn.Module):
    ...
    def __init__(self, in_channels, channels, stride=1, mode='ir'):
        super().__init__()
        ...
        conv2 = []
        if self.mode == 'ip':
            conv2.append(nn.Conv3d(channels, channels,
                         kernel_size=1, stride=1, bias=False))
        conv2.append(nn.Conv3d(channels, channels, kernel_size=3,
                     stride=stride, padding=1, bias=False, groups=channels))
        self.conv2 = nn.Sequential(*conv2)
        self.bn2 = nn.BatchNorm3d(channels)

        self.conv3 = nn.Conv3d(channels, channels *
        ...
        if stride != 1 or in_channels != channels * self.expansion:
            self.downsample = nn.Sequential(
                nn.Conv3d(in_channels, channels * self.expansion,
...
class CSN(nn.Module):
    def __init__(self, block, layers, num_classes, mode='ir'):
        super().__init__()
        ...
        self.in_channels = 64
        # Added code
        self.conv = nn.Conv3d(3, 64, kernel_size=(3, 7, 7), stride=(
            1, 2, 2), padding=(1, 3, 3), bias=False)
        self.bn = nn.BatchNorm3d(64)

        self.conv1 = nn.Sequential(self.conv,
                                   self.bn)

        # self.conv1 = nn.Conv3d(3, 64, kernel_size=(3, 7, 7), stride=(
        #     1, 2, 2), padding=(1, 3, 3), bias=False)
    ...
    def _make_layer(self, block, channels, n_blocks, stride=1):
        ...
        layers = []
        layers.append(
            block(self.in_channels, channels, stride, mode=self.mode))
    ...
    def forward(self, x):
        ...
        print(f'avg_pool: {out.shape}')
        out = out.view(out.size(0), -1)
        print(f'view: {out.shape}')
        # out = self.fc(out)
